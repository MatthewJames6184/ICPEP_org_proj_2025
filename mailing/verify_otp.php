...
if ($otp == $_SESSION['otp']) {
    ...
    $birthday = date('Y-m-d', strtotime($_SESSION['user_birthday']));
    ...
    // Insert new user
    $stmt = $conn->prepare("INSERT INTO user_account (student_number, birthday, email, password, first_name, last_name, year_level, section) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssss", $studNo, $birthday, $email, $password, $firstName, $lastName, $yearLevel, $section);
    if ($stmt->execute()) {
        unset($_SESSION['otp'], $_SESSION['otp_expiry']);
